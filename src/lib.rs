...
pub use agent::{
    ...
    EstimatedImpact, EvidenceAnalyzer, EvidenceGap, ExecutionResult, ExecutionStatus, Executor,
    FileSystemOperation, FileMetadata, GapType, Goal, IntentParseError, LogEntry,
    OperationExecutionState, OperationLog, OperationGuard, OperationPlan, Pipeline,
    PipelineError, PipelineOptions, PipelineResult, PlanError, PlanGenerator, PlanPreview,
    PlanValidationContext, PlanValidator, Policy, PolicyDecision, Precondition, ProposedCategory,
    ProposedOperation, RecoveryResult, Recommendation, RecommendationEngine, RecommendationError,
    RecommendationStrategy, RecommendationWarning, ScopeLock, StructureSummary, TaskAnalysis,
    TaskIntent, TaskIntentParser, UndoConflict, UndoLogEntry, UndoResult, UserDecision,
    ValidatedOperation, ValidationResult, ValidationStatus, ValidationSummary, ValidationWarning,
    WarningType,
};
